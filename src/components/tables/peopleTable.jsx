...
class PeopleTable extends Component {
  columns = [
    { path: "id", label: "Codigo" },
    {
      ...
      label: "Nombre",
      content: (person) => (
        <Link to={`/obrero/${person.id}`} className="text-dark font-weight-bold"> {person.first_name} </Link>
      ),
    },
    {
      path: "last_name",
      label: "Apellidos",
      content: (person) => (
        <Link to={`/obrero/${person.id}`} className="text-dark font-weight-bold"> {person.last_name} </Link>
      ),
    },
    {
      path: "credential",
